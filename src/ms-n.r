...
# save relevant data 
write.table(idx_vec, "../pro-files/data/gen/ms-sa-n-idx-vec.csv", sep="\t", col.names=F, row.names=F)

wl_const_vec <- c(0, wl_vec)
par_vec <- ms.par.vec(idx_vec)
par_mat <- cbind(wl_const_vec[idx_vec], par_vec)
write.table(par_mat, "../pro-files/data/gen/ms-sa-n-par.csv", sep="\t", col.names=F, row.names=F)
