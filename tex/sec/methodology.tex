...
%		\]
	...
	\label{ssec:simulation}
		...
		In this way we, we can compare the $\widehat{\m{SPSE}}^{(\widetilde{\s{M}})}$ with the \enquote{true} model's $\m{SPSE}$ in 19 situations and on the same r pseudo-observations for each.
