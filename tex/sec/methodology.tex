...
%		\end{alignat*}
	...
	\label{ssec:mallows-C_p}
		...
		\[
			\m{SPSE} \define \sum_{i = 1}^n \expect \curvb{Y_{n+i} - \hat{Y}_i^{(M)}}^2,
		\]
		which simplifies to % to be elaborated
		\[
			\m{SPSE} = n\sigma^2 + \m{p}\sigma^2
		\]
		As $\sigma^2$ is unobservable, we have to estimate it so that our estimator for the true $\m{SPSE}$ becomes
		\[
			\widehat{\m{SPSE}} \define \norm{Y - \mathbb{X}^{(M)}\hat{\beta}^{(M)}(Y)}^2_2 + 2\m{p}\curvb{\hat{\sigma}^2}^{(\overline{\Lambda})}
		\]
		...
		\]
		The minimization this value is equivalent to the minimization of the sum of predicted squared errors ($\widehat{\m{SPSE}}$).
	...
	\label{ssec:simulation}
		...
		In the following section, we assume that $\mu$ and $\sigma^2$ are known. true $\m{SPSE}$
		% rationale
		% idea
		% implementation
		To generate pseudoobservations, we use the selected model's estimators instead $\mu$ and $\sigma^2$ by adding a Gaussian error term to the predictions by the selected model.
		Then we forget both and proceed to select a best model and estimate its parameters for each simulation.
		We then proceed to calculate the $\widehat{\m{SPSE}}$ for each simulation.
		We then compute the arithmetic mean of the $\widehat{\m{SPSE}}$ and compare it to the known true $\m{SPSE}$.
		
		% subsampling along the lines of data size and full model size
