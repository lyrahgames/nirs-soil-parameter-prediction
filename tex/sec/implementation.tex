...
\label{sec:implementation}
	...
	\label{ssec:preprocessing}
		 ...
		 From \ref{sec:methodology} becomes that we have plenty of computations to perform. % how much?
		 We need to compute the RSS in order to compute the $C_\m{p}$.
		 To compute the RSS we have to compute the $\hat{Y}$.
		 difficult		 % hat matrix
		 better: % compute \beta
		 		% matrix inversion is bad => QR solving (X'X)\beta = X'y
